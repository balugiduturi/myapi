...
module.exports = function (req, res) {
    ...
    (async function () {
        try {
            ...
            function getDates(collection_name, fpId) {
                return new Promise(async (resolve, reject) => {
                    try {
                        var collection = "";
                        let doc = "";
                        collection = local_master_db.collection(collection_name);
                        doc = await collection.findOne({"_id": ObjectId(fpId)});
                        if (!doc) {
                            reject(new Error(`There is no listing with this fp_id ==== ${fpId}`));
                            return;
                        } else {
                            if (doc.dates && doc.dates.dp_moved) {
                                resolve(doc.dates.dp_moved);
                            } else {
                                reject(new Error("unable to find dates from local"));
                                return;
                            }
                        }
                    } catch (E) {
...
//                                doc = docs[doc_index];
//                                let source = doc._source;
//                                let mongo_date = await getDates('leads_with_url', fpId);
//                                let elastic_date = source.dates ?
//                                        (source.dates.dp_moved ? source.dates.dp_moved.sec : "") : "";
//                                if (!elastic_date && !mongo_date) {
//                                    reject(new Error("unable to find dates from local or live"));
//                                    return;
//                                } else {
//                                    console.log(elastic_date);
...
//                            }
            ...
            dbLogger.logRespTime(req.id, new Date());
            dbLogger.logRespTime(req.id, new Date());
            dbLogger.setLogger(req.id, "RES_BODY", new Date(), E.message);
            res.send({
