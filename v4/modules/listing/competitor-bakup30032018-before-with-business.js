...
function clientSearch(inputId, catType) {
    return new Promise(async function (resolve, reject) {
        try {
            var result = await client.search({index: elastic_indexes["leads_with_url"], type: elastic_types["projects"], body: prepareEalsticQuery(inputId)});
            if (result && result.hits.total > 0) {
                ...
                if (!doc["country_code"] || !doc["lat"] || !doc["hostname"])
                    resolve({"err_msg": "Field not available for this listing"});
                else
                    resolve(doc);
            } else {
                resolve(null);
