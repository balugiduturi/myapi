...
function prepareLocationQuery(location_scope, location_keyword) {
...
function getBusinessName(input) {
    dbLogger.setLogger(input.reqId, "BUSINESSNAME STARTS", new Date(), "BUSINESSNAME STARTS");
        ...
        resolve((async function () {
            try {
                ...
                if (input.location_keyword && input.location_scope) {
                    sub_query.push(prepareLocationQuery(input.location_scope, input.location_keyword));
                }
...
//                        categories.push({name: value.category_ids.hits.hits[0]._source.google_category , count: value.doc_count, category_id: value.category_ids.hits.hits[0]._source.category_id});
                    ...
                    dbLogger.setLogger(input.reqId, "BUSINESSNAME ENDS", new Date(), "BUSINESSNAME ENDS");
                    return {
    ...
    return new Promise((resolve, reject) => {
        resolve((async function () {
            try {
                ...
                dbLogger.setLogger(input.reqId, "WEBSITE STARTS", new Date(), "WEBSITE STARTS");
                ...
                if (input.location_keyword && input.location_scope) {
                    subQuery.push(prepareLocationQuery(input.location_scope, input.location_keyword));
                }
...
//                    let websites = _.pluck(bucketArray, 'key');
                    ...
                    dbLogger.setLogger(input.reqId, "WEBSITE ENDS", new Date(), "WEBSITE ENDS");
                    return {
    ...
    return new Promise((resolve, reject) => {
        resolve((async function () {
            try {
                ...
                dbLogger.setLogger(input.reqId, "PRODUCT STARTS", new Date(), "PRODUCT STARTS");
...
//                        categories.push({name: value.category_ids.hits.hits[0]._source.google_category , count: value.doc_count, category_id: value.category_ids.hits.hits[0]._source.category_id});
                    ...
                    if (products.length === 0) {
                        ...
                        dbLogger.setLogger(input.reqId, "PRODUCT ENDS", new Date(), "PRODUCT ENDS");
                        return {
    ...
    return new Promise((resolve, reject) => {
        resolve((async function () {
            try {
                ...
                dbLogger.setLogger(input.reqId, "CATEGORY STARTS", new Date(), "CATEGORY STARTS");
...
//                            console.log(util.inspect(havingbucketArray[havingbucket], false, null));
                    ...
                    dbLogger.setLogger(input.reqId, "CATEGORY ENDS", new Date(), "CATEGORY ENDS");
...
//listing_source
                    ...
                    for (let scopevalue in input.scope) {
                        console.log("------",input.scope[scopevalue]);
                    ...
                    input.reqId = req.id;
                    dbLogger.setLogger(input.reqId, "AUTOSUGGEST STARTS", new Date(), "AUTOSUGGEST STARTS");
                    let result = await Promise.all([getBusinessName(input), getWebsite(input), getCategories(input), getProducts(input)]);
                    dbLogger.setLogger(input.reqId, "AUTOSUGGEST ENDS", new Date(), "AUTOSUGGEST ENDS");
                    let finalResult = [];
