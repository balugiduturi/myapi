    ...
    return new Promise((resolve, reject) => {
        resolve((async function () {
            try {
                ...
                if (input.locality) {
                    sub_query.push(prepareLocationQuery("locality", input.locality));
                }
                if (input.region) {
                    sub_query.push(prepareLocationQuery("region", input.region));
                }
                if (input.postal_code) {
                    sub_query.push(prepareLocationQuery("postal_code", input.postal_code));
                }
    ...
    return new Promise((resolve, reject) => {
        resolve((async function () {
            try {
                ...
                if (input.locality) {
                    subQuery.push(prepareLocationQuery("locality", input.locality));
                }
                if (input.region) {
                    subQuery.push(prepareLocationQuery("region", input.region));
                }
                if (input.postal_code) {
                    subQuery.push(prepareLocationQuery("postal_code", input.postal_code));
                }
        ...
        resolve((async function () {
            try {
                ...
                if (input.listing_source === 'private') {
                    categoryField = 'business.buzz_category_name';

                    subQuery.push({"match_phrase": {"business.buzz_category_name.autocomplete": category_keyword_full}})
                    subQuery.push({"term": {"business.buzz_partner_id": `${input.partner_id}`}})
                    if (category_split_length > 1) {
                        subQuery.push({"match_phrase": {"business.buzz_category_name": category_keyword_modified}})
                    }
                } else {
                    /*
                     * public
                    ...
                    subQuery.push({"match_phrase": {"address.country_code": `${input.country_code}`}})
...
//                        subQuery.push({"missing": {"field": "category_source"}})
                    } else if (input.category_type === 'neustar') {
                        /*
                         * for other category_type ex:neustar
                         */
                        categoryField = 'business.neustar_category_lables';
                        subQuery.push({"match_phrase": {"business.neustar_category_lables.autocomplete": category_keyword_full}})
                        if (category_split_length > 1) {
                            subQuery.push({"match_phrase": {"business.neustar_category_lables": category_keyword_modified}})
                        }
...
//                    else if(input.category_type === 'predicted'){
//                        /*
//                         * for other category_type ex:neustar
//                         */
//                         categoryField='business.neustar_category_lables';
...
//                });
                result = await client.search({index: elastic_indexes['leads_with_url'] + "," + elastic_indexes['leads_without_url'],
                    type: elastic_types['leads_with_url'] + "," + elastic_types['leads_without_url'],
                    body: getFullQueryCategory(subQuery, input.size_limit, category_keyword_full, categoryField)
                });
                if (result.hits.total !== 0 && result) {
                    let likebucketArray = result.aggregations.bnames.buckets.like.bname.buckets;
                    let havingbucketArray = result.aggregations.bnames.buckets.having.bname.buckets;
...
function getFullQueryCategory(andQueryArray, size, category_keyword_full, categoryField) {
//    console.log("andQieryArray ",andQieryArray)
    ...
    eachQuery["match_phrase"][`${categoryField}`] = category_keyword_full;
        ...
        "aggs": {
            "bnames": {
                "filters": {
                    "filters": {
                        "like": eachQuery,
                        "having": {"not": eachQuery}
                    }
                },
                "aggs": {
                    "bname": {
                        "terms": {
                            "field": `${categoryField}.sort`,
                            "size": size,
                            "order": {"_term": "asc"}
                        }
//                        ,                        "aggs": {"category_name": {"top_hits": {"_source": {"include": [`${categoryField}`]}, "size": 1}}}
                    }
                }
            }
        }
