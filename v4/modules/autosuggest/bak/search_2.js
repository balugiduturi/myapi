            ...
            try {
                ...
                if (result && result.hits.total !== 0) {
                    ...
                    if (bnames.length < input.size_limit) {
                        for (havingbucket in havingbucketArray) {
                            bnames.push({name: havingbucketArray[havingbucket].key, count: havingbucketArray[havingbucket].doc_count});
                            if (bnames.length == input.size_limit) {
                                break
                            }
                        }
                    ...
                    result = await client.search({index: elastic_indexes['leads_with_url'],
                        ...
                        body: {
                            ...
                            "query": {
                                ...
                                "bname": {
                                    "terms": {
                                        "field": "domain_data.hostname.raw"
                ...
                "bnames": {
                    "filters": {
                        ...
                        "bname": {
                            ...
                            "aggs": {"category_ids": {"top_hits": {"_source": {"include": ["category_id"]}, "size": size}}}
                        }
                    }
                }
            }
