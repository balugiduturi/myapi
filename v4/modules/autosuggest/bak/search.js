    ...
    return new Promise((resolve, reject) => {
        resolve((async function () {
            try {
                ...
                var product_keyword_full = input.keyword;
                ...
                if (result && result.hits.total !== 0) {
                    ...
                    _.each(likebucketArray, (value) => {
                        ...
                        products.push({name: product_with_ids[value.key].name, count: value.doc_count,product_id:product_with_ids[value.key].id});
//                        categories.push({name: value.category_ids.hits.hits[0]._source.google_category , count: value.doc_count, category_id: value.category_ids.hits.hits[0]._source.category_id});
                    });
                    if (products.length < input.size_limit) {
                        for (var havingbucket in havingbucketArray) {
                            products.push({name: product_with_ids[havingbucketArray[havingbucket].key].name, count: havingbucketArray[havingbucket].doc_count, product_id:product_with_ids[havingbucketArray[havingbucket].key].id});
                            if (products.length === input.size_limit) {
                                break;
                            }
                        }
...
//                    let bnames = _.pluck(bucketArray, 'key');
                    ...
                    if (categories.length < input.size_limit) {
                        for (var havingbucket in havingbucketArray) {
                            ...
                            if (isGoogle){
                            categories.push({name: havingbucketArray[havingbucket].category_ids.hits.hits[0]._source.google_category, count: havingbucketArray[havingbucket].doc_count, category_id: havingbucketArray[havingbucket].category_ids.hits.hits[0]._source.category_id});
                        }
                        else{
                            categories.push({name: havingbucketArray[havingbucket].category_ids.hits.hits[0]._source.google_name, count: havingbucketArray[havingbucket].doc_count, category_id: havingbucketArray[havingbucket].category_ids.hits.hits[0]._source.category_id});
                        }
                            if (categories.length === input.size_limit) {
                                break
                            }
