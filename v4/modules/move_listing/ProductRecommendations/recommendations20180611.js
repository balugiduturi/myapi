...
module.exports = function (doc) {
    return new Promise(async function (resolve, reject) {
        try {
            ...
            if (!doc) {
                let text = `doc not sent to recommenations file`;
...
//            recommended_products.push({product_id: 3, elements: SEO_elements_absense_details.elements,presenece_count:SEO_elements_absense_details.count})
            recommended_products.push({product_id: 3, elements: SEO_elements_absense_details.elements})
        }
        return;
    } catch (Exception) {
        throw Exception;
...
function checkSEOMaintinence(elements_values_obj, recommended_products) {
    ...
    try {
        ...
        let infrastructure_elements_presence_details = get_elements_presents_details(elements_values_obj, InfraStructure_elements)
        notpresent_elements = notpresent_elements.concat(SEO__Maintinence_elements_present_details.elements);
        notpresent_elements = notpresent_elements.concat(infrastructure_elements_presence_details.elements);
