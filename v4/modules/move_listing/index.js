...
//                                var sub_ref_id = sub_doc_mongo_id.toString();
        ...
        function getsubDoc(collection, query) {
            return new Promise(async function (resolve, reject) {
                try {
                    ...
                    var documnent = await data_collection.find(query).sort({$natural: -1}).limit(1).toArray();
                    resolve(documnent);
                } catch (E) {
                    reject(E);
                }
        ...
        try {
            ...
            if (masterCollection) {
                ...
                var formatted_doc;
                /* assigning doc and which is not disturb original i.e leads_documnt*/
                ...
                console.log("********* Master Collection " + masterCollection + "Mongo Move Compelted ***************** doc_id :", doc_id)
                ...
                if (update_collections && update_collections.length > 0) {
                    ...
                    dbLogger.move_data_api_log_setLogger(reqId, "MOVE-SUB-STARTS", new Date(), "MOVE-SUB-STARTS");
                    await moveSubCollections(req_fp_id, update_collections, req_reference_id, reqId, master_doc_version);
                    dbLogger.move_data_api_log_setLogger(reqId, "MOVE-SUB-ENDS", new Date(), `${(new Date() - startTime)}`);
                    resolve({message: "process completed", error: null, moved_info: moved_info, "version_info": version_info});

                    /*
                     * Moving background collecttions 
                     * 
                     */
                    var posts_collection = ["facebook_posts_data", "brand_mentions_data", "twitter_posts_data", 'google_posts_data'];
