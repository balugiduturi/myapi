...
module.exports = function (input) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
                    ...
                    try {
                        ...
                        if (!temp_indexes[`${collection}`]) {
                            try {
                                ...
                                dbLogger.setLogger(input.id, "SIGNAL_ERROR", new Date(), E.message);
                                resolve(null);
                            }
            ...
            if (!input) {
                dbLogger.setLogger(input.id, "ERROR", new Date(), "input must contain body");
                dbLogger.setLogger(input.id, "RES_BODY_SIGNALS_SERVICE", new Date(), {
                    status: 412, error: "request must contain body",
                    meta: {
                        req_reference_id: input.id
                    }
                });
                reject(new Error("input must contain body"));
                return;
            }

            if (input.fp_id) {

                if (!input.hasOwnProperty("signals")) {
                    reject(new Error("param signals are missiing"));
                }
                if (!input.hasOwnProperty("custom_args")) {
                    reject(new Error("parama custom_args are missing"));
                ...
                for (let signal in input.signals) {
                    ...
                    var result = await checkFpId(input.id, element_path, collection, input.fp_id);
                    if (result === null) {
                ...
                dbLogger.setLogger(input.id, "RES_BODY_SIGNALS_SERVICE", new Date(), util.inspect({
                    data,
                    meta: {
                        req_reference_id: input.id
                    }
                }));
                dbLogger.logRespTime(input.id, new Date());
                resolve(data);
            } else {
                dbLogger.logRespTime(input.id, new Date());
                dbLogger.setLogger(input.id, "RES_BODY_SIGNALS_SERVICE", new Date(), {
                ...
                reject(new Error("params fp_id is mandatory"));
            }
